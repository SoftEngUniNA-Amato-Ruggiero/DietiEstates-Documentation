...
\begin{itemize}
    \item \textbf{auth-service}: gestisce l'autenticazione tramite Oauth. Espone i metodi per login e logout.
    \item \textbf{user-state-service}: mantiene informazioni relative all'utente autenticato, come email, ruoli e preferenze di notifica.
    \item \textbf{backend-client-service}: gestisce le chiamate HTTP al backend.
    \item \textbf{geoapify-client-service}: interfaccia con l'API di Geoapify per il reverse geocoding. Converte coordinate geografiche in indirizzi leggibili.
    \item \textbf{insertion-preview-service}: gestisce l'apertura del modal di riepilogo delle inserzioni.
    \item \textbf{saved-search-service}: coordina la comunicazione tra componenti relativi alle ricerche salvate.
...
\begin{itemize}
    \item \textbf{auth-guard}: autorizza l'accesso a una route solo agli utenti autenticati. Attivato per le route /profile, /agency, /dashboard, /logout.
    \item \textbf{noauth-guard}: autorizza l'accesso a una route solo agli utenti non autenticati. Attivato per la route /login.
    \item \textbf{agent-guard}: autorizza l'accesso a una route solo agli utenti con il ruolo di agente. Attivato per la route /dashboard.
    \item \textbf{manager-guard}: autorizza l'accesso a una route solo agli utenti con il ruolo di agente. Attivato per la route /agency.
\end{itemize}
